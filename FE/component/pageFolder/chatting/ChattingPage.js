...
import {
    SafeAreaView, View, TextInput, Button, FlatList, Text, StyleSheet, ScrollView, KeyboardAvoidingView, Keyboard, Image
} from 'react-native';
...
function ChattingPage(props) {
    ...
    const renderItem = ({ item }) => (
        <View style={[styles.messageContainer, item.isMyMessage ? styles.myMessageContainer : styles.otherMessageContainer]}>
            {!item.isMyMessage && <Image source={require('../../img/gpt.png')} style={styles.image} />}
            <View style={[styles.message, item.isMyMessage ? styles.myMessage : styles.otherMessage]}>
                <Text>{item.text}</Text>
            </View>
        </View>
    );
    ...
    const sendMessage = () => {
        const newMessage = { text, isMyMessage: true }; // isMyMessage는 현재 사용자의 메시지를 구분하기 위해 추가했습니다. 이 값을 적절하게 설정해야 합니다.
    ...
    const sendQuestion = () => {
        instance
            .post(`/api/chat-gpt/question`, {
                "question": text
            })
            .then((response) => {
                let GPTMessage = response.data.choices


                GPTMessage.map((messages,i)=>{
                    let text = messages.text.replace('\n\n', '');
                    let newMessage = {  text, isMyMessage: false};
                    setMessages(prevMessages => [...prevMessages, newMessage]);
                });
